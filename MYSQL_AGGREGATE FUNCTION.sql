...
select Region, SUM(Quantity) AS Total_sold_product from Sales GROUP BY Region;

#3.	Write a query to get the average sales amount per product. 

select ProductID, avg(UnitPrice) as AverageSalesAmount from Sales group by ProductID;
...
select SaleID,Region,ProductID,avg(Quantity)as Total_sold_product from Sales group by SaleID,Region,ProductID;
...
select DATE_FORMAT(SaleDate, "%Y-%m") as month, SUM(Quantity*UnitPrice)
 AS total_sales from sales group by
 DATE_FORMAT(SaleDate, "%Y-%m") order by month;

#12.Find the regions where the average unit price is more than 200. 

select Region,avg(UnitPrice) as average_unit_price
 from Sales group by Region having avg(UnitPrice)>200;

#13.Write a query to get the minimum and maximum quantity sold per region. 

select Region,max(Quantity) as max_quantity,
min(Quantity)as min_quantity from Sales group by Region;
...
select ProductID,(Quantity*UnitPrice)as total_sales
 from Sales where (Quantity*UnitPrice)>1500;
